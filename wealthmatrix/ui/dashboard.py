...
class DashboardWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        self.bank_scroll.setFixedHeight(S(155))
        ...
        lay.addWidget(bank_panel)

        # ── 現金面板（緊湊橫條）────────────────────────────────────
        cash_panel = CpPanel(accent=CP["green"])
        cash_panel.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        cash_hbox = QHBoxLayout(cash_panel)
        cash_hbox.setContentsMargins(S(12), S(8), S(12), S(8))
        cash_hbox.setSpacing(S(12))
        cash_hbox.addWidget(section_label("現金"))
        self.cash_lbl = QLabel("NT$0")
        self.cash_lbl.setObjectName("cash_val")
        cash_hbox.addWidget(self.cash_lbl)
        cash_hbox.addStretch()
        ...
        cash_hbox.addWidget(self.cash_input)
        cash_hbox.addWidget(set_cash_btn)
        lay.addWidget(cash_panel)
        ...
        self.stock_scroll.setFixedHeight(S(210))
        ...
        self.goals_scroll.setFixedHeight(S(265))
